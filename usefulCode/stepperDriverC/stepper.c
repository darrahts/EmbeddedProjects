...
void __reset_limit_flags__(struct Stepper* stepper)
{
	if(stepper->direction == 'l')
		{
			stepper->previousDirection = 'l';
			stepper->direction = 'r';
		}
	else if(stepper->direction == 'r')
		{
		stepper->previousDirection = 'r';
		stepper->direction = 'l';
		}

	printf("%c\n", stepper->direction);
	step(stepper, 8, stepper->direction);
	...
	char temp = stepper->previousDirection;
	stepper->previousDirection = stepper->direction;
	stepper->direction = temp;
}
